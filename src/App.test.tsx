...
describe('single-page planner interactions', () => {
  ...
  it('starts with the structured Quicken inputs and shows the interactive financial story', async () => {
    const user = userEvent.setup()
    render(<App />)
    expect(screen.getByText('01. あなたと家族 (About You)')).toBeInTheDocument()
    ...
    expect(screen.getByText('04. 運用資産・NISA/iDeCo (Investments)')).toBeInTheDocument()
  ...
  it('switches the complete interface from Japanese to Chinese', async () => {
    ...
    expect(screen.getByText('01. 个人与家庭 (About You)')).toBeInTheDocument()
