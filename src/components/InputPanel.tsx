                ...
                <div className="flex items-center justify-between gap-2 border-b border-slate-200/60 pb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      className="h-7 text-xs font-semibold bg-transparent border-0 focus:bg-white focus:border-slate-300 p-1 w-28"
                    />
                  </div>
                  {adult.role === 'spouse' ? (
                    <Button
            ...
            {plan.children.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {plan.children.map((child) => (
                  <div key={child.id} className="flex items-center justify-between gap-2 p-2 rounded-lg border border-slate-200/80 bg-slate-50/50">
                    <Input
                      aria-label={`${child.name} ${copy.name}`}
                      value={child.name}
                      onChange={(event) => updateChild(child.id, { name: event.target.value })}
                      className="h-7 text-xs font-semibold bg-white border-slate-200 w-28"
                    />
                    <div className="flex items-center gap-1.5">
                      <Input
                        ...
                        className="h-7 w-14 text-xs font-semibold bg-white border-slate-200 text-right"
                      />
                      <span className="text-xs text-slate-400 font-medium">{copy.age}</span>
                    </div>
                    <Button
                      type="button"
                      variant="ghost"
              ...
              >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <Input
                    ...
                    className="h-9 text-xs font-semibold bg-white border-slate-200 focus:border-sky-500 rounded-lg"
                  />
                </label>
                ...
                <div className="flex items-center justify-between gap-3 pt-1 border-t border-slate-100">
                  <label className="flex items-center gap-2 text-xs text-slate-600 font-medium">
                    ...
                    <select
                      ...
                      className="h-8 text-xs font-medium bg-white border border-slate-200 rounded-lg px-2 text-slate-700 focus:border-sky-500 outline-none"
                    >
